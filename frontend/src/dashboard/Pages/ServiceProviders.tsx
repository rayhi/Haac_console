  ...
  { field: "createdAt", headerName: "Date Created", flex: 1 },
  {
    ...
    renderCell: (params) => (
      <Button
        ...
        size="small"
        // @ts-ignore
        onClick={() => navigate(`/service-providers/${params.id}`)}
      >
        View More
      </Button>
    ),
...
const providerRows: GridRowsProp = [
...
export default function ServiceProviders() {
  ...
  const columnsWithActions = providerColumns.map(col =>
    col.field === 'actions' ? {
      ...col,
  ...
  return (
    <Layout
      ...
      actions={
        <CustomButton startIcon={<AddIcon />}>Add Provider</CustomButton>
      }
    >
      <Grid
        container
              ...
              <Grid size={{ md: 12, xs: 12 }}>
                ...
                >
                  ...
                  <Stack direction="row" spacing={1}>
                    <Tooltip title="Export All Providers">
                      <span>
                        <IconButton onClick={() => exportToCSV([...providerRows], 'providers.csv')} sx={{ display: 'flex', alignItems: 'center', height: 36 }}>
                    ...
                    <Tooltip title="Export Selected Providers">
                      <span>
                        <IconButton
                          onClick={() => {
                            const selectedRows = providerRows.filter(row => selected.includes(row.id));
                            exportToCSV(selectedRows, 'selected_providers.csv');
                          }}
